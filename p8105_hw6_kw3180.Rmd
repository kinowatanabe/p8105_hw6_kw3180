...
weather_df |> 
  ...
  geom_point() +
  labs(title = "Scatterplot of tmin vs tmax", 
...
weather_df |> 
  ...
  geom_point() +
  labs(title = "Scatterplot of prcp vs tmax", 
       x = "prcp (tenths of mm)", 
       y = "tmax (tenths of degrees C)")
```

* Because prcp doesn’t have a linear relationship with tmax, the assumptions for linear regression are violated. We can bootstrap to estimate using empirical distribution without relying on these parametric assumptions.
...
* We are 95% confident that the true r^2 value lies between 0.89 and 0.92. We are 95% confident that the true beta1/beta2 value lies between -5616.45 and 4586.92.
...
Proposed hypothesis model process: 

I am aiming for a theory-driven hypothesis proposal based on published literature. Based on "The determinants of birth weight," they asserted, "The results show that the sex of the baby, parity, maternal smoking during the pregnancy, maternal height, weight, marital status, and race... were all important and significant factors." Thus, I will propose a model with `baby_sex`, `parity`, `mom_smoke_per_day`, `mom_height_in`, `mom_wt_gain_lb`, and `mother_race` as predictors to birthweight.

[The determinants of birth weight paper](https://pubmed.ncbi.nlm.nih.gov/7114129/)
...
```
Residual Diagnostics model process: 

* The first plot shows that the residuals for the hypothesized model is generally normally distributed, symmetric, and centered at 0. There is noticeable slight skewing on the left-hand side.
* The "Residuals vs Fitted Values for Hypothesis Model" shows random scatter around the red line at 0. This suggests that the linearity assumption is not violated.
...
cv_df <- cv_df |> 
  ...
  mutate(
    rmse_hypothesis_fit = map2_dbl(hypothesis_fit, test, rmse),
...
cv_df |> 
  ...
  mutate(model = factor(model, 
                        levels = c("fit_interactions", "fit", "hypothesis_fit"))
  ) |> 
...
* The RMSEs suggest that the `fit_interactions` model performs better than both the `fit` and `hypothesis_fit` models. The hypothesized model, which included predictors `baby_sex`, `mom_smoke_per_day`, `mom_height_in`, `mom_wt_gain_lb`, and `mother_race`, had the largest RMSE, likely because it lacks key baby-specific variables. This model emphasizes maternal characteristics and their impact on birthweight, rather than direct measurements of the baby. The focus on the mother, rather than the baby (or dyad, for that matter), makes the model more susceptible to prediction error. In contrast, the `fit` model includes gestational age and baby’s length, enabling it to capture more variance (variables are direct indicators of the baby’s size). The lower RMSE for the `fit_interactions` model indicates that including interactions between baby-specific measures allows the model to account for joint effects on birthweight, improving the model's predictions.
