...
bwt_df |> 
  ...
  labs(
    title = "QQ Plot of Residuals for Hypothesis Model",
    x = "Theoretical Quantiles",
    y = "Sample Quantiles"
  )
```

* The first plot shows that the residuals for the hypothesized model is generally normally distributed, symmetric, and centered at 0. There is noticeable skewing on the left-hand side.
* The "Residuals vs Fitted Values for Hypothesis Model" is used to assess linearity and homoscedasticity. The roughly oval shape centered around 0 indicates the model is mostly linear, but there is some non-constant variance (heteroskedasticity), with slightly more spread in residuals at lower fitted birthweights..
