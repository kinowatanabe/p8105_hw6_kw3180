...
- Because prcp doesn’t have a linear relationship with tmax, the
  assumptions for linear regression are violated. We can bootstrap to
  estimate using empirical distribution without relying on these
...
- The distribution after bootstrapping for β1 hat/ β2 hat is extremely
  ...
  large 95% CI = (-5616.45, 4586.92). This occurs because the beta
...
|------------:|------------:|--------------:|--------------:|
|      0.8942 |      0.9277 |     -5616.445 |      4586.923 |

- We are 95% confident that the true r^2 value lies between 0.89 and
  0.92. We are 95% confident that the true beta1/beta2 value lies
  between -5616.45 and 4586.92.
...
were all important and significant factors.” Thus, I will propose a
model with `baby_sex`, `parity`, `mom_smoke_per_day`, `mom_height_in`,
`mom_wt_gain_lb`, and `mother_race` as predictors to birthweight.

[The determinants of birth weight
paper](https://pubmed.ncbi.nlm.nih.gov/7114129/)
...
- The first plot shows that the residuals for the hypothesized model is
  ...
  noticeable slight skewing on the left-hand side.
- The “Residuals vs Fitted Values for Hypothesis Model” shows random
  scatter around the red line at 0. This suggests that the linearity
...
- The RMSEs suggest that the `fit_interactions` model performs better
  than both the `fit` and `hypothesis_fit` models. The hypothesized
  model, which included predictors `baby_sex`, `mom_smoke_per_day`,
  `mom_height_in`, `mom_wt_gain_lb`, and `mother_race`, had the largest
  RMSE, likely because it lacks key baby-specific variables. This model
  emphasizes maternal characteristics and their impact on birthweight,
  rather than direct measurements of the baby. The focus on the mother,
  rather than the baby (or dyad, for that matter), makes the model more
  susceptible to prediction error. In contrast, the `fit` model includes
  gestational age and baby’s length, enabling it to capture more
  variance (variables are direct indicators of the baby’s size). The
  lower RMSE for the `fit_interactions` model indicates that including
  interactions between baby-specific measures allows the model to
  account for joint effects on birthweight, improving the model’s
  predictions.
